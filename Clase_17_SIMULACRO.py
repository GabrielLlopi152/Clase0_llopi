...
from biblioteca_funciones_simulacro import *
import json
...
while continuar == True:
    ...
    if json_cargado == False:
        registro_vuelos = A_cargar_json(opcion)
        json_cargado = True
        ...
        match opcion:
            ...
            case 'B' | 'b':
                B_alta(registro_vuelos)
            case 'C' | 'c':
                C_modificar(registro_vuelos)
            case 'D' | 'd':
                D_baja(registro_vuelos)
            case 'E'| 'e':
                E_listar(registro_vuelos)
            case 'F'| 'f':
                ...
                while submenu_activo:
                    ...
                    match opcion:
                        case '1':
                            ...
                            bandera = False
                            for pasajero in registro_vuelos['pasajeros']:
                            ...
                            mensaje = 'El precio con {0} precio es {1}'.format(opcion, pasajero_precio)
                            print(mensaje)
                        ...
                        case '3':
                            ...
                            while opcion != 'asc' and opcion != 'desc':
                                opcion = input('asc | desc: ')
                            ordenar_lista_diccionarios(registro_vuelos['pasajeros'],'Fecha', opcion)
                        ...
                        case '4':
                            actualizar_json('archivo_parcial.json', registro_vuelos)
                        case '5':
                            
                            with open('archivo_parcial.csv', 'w') as file:
                                file.write(str(lista_pasajero_precio))
